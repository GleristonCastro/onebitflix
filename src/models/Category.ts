import { DataTypes, Optional, Model } from 'sequelize';
import { sequelize } from '../database';

export interface Category {
  id: number
  name: string
  position: number
};

//Agora vamos criar os atributos de criação de uma categoria no banco de dados.
export interface CategoryCreationAttributes extends Optional<Category, 'id'> {};
//Agora a intància do model Category, é uma mistura do model do Sequelize com a nossa interface Category
export interface CategoryInstance extends Model<Category, CategoryCreationAttributes>, Category{};

export const Category = sequelize.define<CategoryInstance, Category>('Category', {
  id: {
    allowNull: false,
    autoIncrement: true,
    primaryKey: true,
    type: DataTypes.INTEGER
  },
  name: {
    allowNull: false,
    type: DataTypes.STRING
  },
  position: {
    allowNull: false,
    type: DataTypes.INTEGER
  }
});